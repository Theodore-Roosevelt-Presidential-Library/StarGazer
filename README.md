...
Nothing in the stories or events requires touching the widget code. Edit
`stargazer-content.js`:

- `western` — one paragraph per constellation, keyed by IAU abbreviation.
- `indigenous` — one entry per Native star figure or story: `culture`, `name`, `label` (short
  name drawn on the sky), `translation`, `western`, `hip` (Hipparcos numbers of the stars to mark),
  `con` (constellations that should list the story), `fig` (`ring`, `path`, `cloud`, or `none`),
  `body` (`moon`, `sun`, `venus`, `milkyway`), `story`, `sensitivity`, `confidence`, `sources`, and
  reviewer `notes`. `framing` is the paragraph shown at the top of the Stories panel and under each
  story.
- `showers` — the annual meteor showers (IMO working list).
- `events` — dated sky events; `visibleFromMedora` may be `true`, `false`, or `"partial"`. Add
  entries as the calendar rolls forward; the panel lists everything from today on.
...
into a point cloud). The full research file behind the Indigenous entries, with reviewer notes and
the items deliberately excluded, is `indigenous_star_knowledge.json`.

The widget itself is built from `src/*.js` by `build.py`, which concatenates the parts and inlines
`assets/wordmark.svg`. Both built files are produced by the deploy workflow and are not committed.
To preview locally:
...
research file). Before this content is promoted beyond the Labs subdomain, the Standing Rock Sioux
Tribe, the MHA Nation's cultural office, Nueta Hidatsa Sahnish College, and the Turtle Mountain Band
should be invited to review, correct, and expand it. Lakota orthography with diacritics follows the
standard used at Standing Rock and in the Lakota Language Bowl materials; a fluent speaker should
confirm the reconstructed spellings flagged in the research notes.
