  ...
  function tourCatalog(loc, date) {
    ...
    if (!catalogCache) {
      catalogCache = { con: {}, byHip: {}, lore: [] };
      for (var k in D.con) catalogCache.con[k] = eqVec(D.con[k].ra, D.con[k].dec);
      ...
      for (var l = 0; l < C.indigenous.length; l++) {
        ...
        for (var h = 0; h < L.hip.length; h++) { var si = catalogCache.byHip[L.hip[h]]; if (si == null) continue; var v = eqVec(D.stars[si][1], D.stars[si][2]); cv[0] += v[0]; cv[1] += v[1]; cv[2] += v[2]; n++; }
        catalogCache.lore.push(n ? vnorm(cv) : null);
      }
    }
    ...
    for (i2 = 0; i2 < C.indigenous.length; i2++) {
      ...
      if (la.alt > 3) items.push({ id: Le.id, group: 'Lakota and Native sky', name: Le.label || Le.name, sub: (Le.translation || '').split(' /')[0] + ' · ' + Le.culture.split(',')[0] + ' · ' + where(la), alt: la.alt, vec: lv, j2000: true, sky: 'native', card: { t: 'lore', i: i2 } });
    }
    ...
    for (var ck in D.con) { var cvv = catalogCache.con[ck], ca = aa(mulMat(M, cvv)); if (ca.alt > 12 && (brightest[ck] != null || (D.art && D.art[ck]))) items.push({ id: 'con-' + ck, group: 'Constellations', name: D.con[ck].n, sub: (C.origins && C.origins[ck] ? C.origins[ck].era + ' · ' : '') + where(ca), alt: ca.alt, vec: cvv, j2000: true, sky: 'western', card: { t: 'con', k: ck } }); }
    ...
    for (i2 = 0; i2 < D.dsos.length; i2++) { var ds = D.dsos[i2]; if (!picks[ds[1]]) continue; var dv = eqVec(ds[3], ds[4]), da = aa(mulMat(M, dv)); if (da.alt > 8) items.push({ id: 'dso-' + i2, group: 'Stars and deep sky', name: ds[1] === 'GalCtr' ? 'Heart of the Milky Way' : ds[0], sub: where(da), alt: da.alt, vec: dv, j2000: true, sky: null, card: { t: 'dso', i: i2 } }); }
