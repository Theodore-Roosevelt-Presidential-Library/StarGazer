...
Also copies the Stellarium constellation illustrations into art/. Requires only numpy. Usage:  python3 tools/build_data.py
"""
...
# Constellation figure illustrations: Stellarium "western" sky culture, drawn by Johan Meuris (Free Art License);
# anchor data CC BY-SA. https://github.com/Stellarium/stellarium-skycultures
ART_COMMIT = '014fbb5e59233d133c22f9811af96b67d05a95c9'
ART_BASE = 'https://raw.githubusercontent.com/Stellarium/stellarium-skycultures/%s/western/' % ART_COMMIT
...
def fetch(name, base=BASE, raw=False):
    os.makedirs(CACHE, exist_ok=True)
    p = os.path.join(CACHE, name.replace('/', '_'))
    if not os.path.exists(p):
        print('downloading', name)
        urllib.request.urlretrieve(base + name, p)
    return p if raw else json.load(open(p, encoding='utf-8'))


def build_art(star_pos):
    """Copy the 85 figure illustrations into art/ and return {IAU: [file, w, h, [[x, y, ra, dec] x3]]}."""
    import shutil
    idx = fetch('index.json', ART_BASE)
    os.makedirs(ART_DIR, exist_ok=True)
    extra = None
    out = {}
    for c in idx['constellations']:
        im = c.get('image')
        if not im or not c.get('iau'):
            continue
        anchors = []
        for a in im['anchors']:
            hip = a['hip']
            if hip not in star_pos:
                if extra is None:   # a few anchors are fainter than mag 6: look them up in the mag-8 catalog
                    extra = {f['id']: f['geometry']['coordinates'] for f in fetch('stars.8.json')['features']}
                if hip not in extra:
                    print('  anchor HIP', hip, 'not found for', c['iau']); anchors = None; break
                ra, dec = extra[hip]; star_pos[hip] = (norm(ra), dec)
            ra, dec = star_pos[hip]
            anchors.append([a['pos'][0], a['pos'][1], round(ra, 3), round(dec, 3)])
        if not anchors:
            continue
        fname = os.path.basename(im['file'])
        src = fetch(im['file'], ART_BASE, raw=True)
        shutil.copyfile(src, os.path.join(ART_DIR, fname))
        out[c['iau']] = [fname, im['size'][0], im['size'][1], anchors]
    return out
...
def main():
    ...
    art = build_art(star_pos)
    out = {'stars': S, 'con': C, 'dsos': D, 'mw': pts, 'bnd': B, 'art': art}
    ...
    print('wrote', os.path.relpath(OUT), round(len(js) / 1024), 'KB —', len(S), 'stars,', len(C), 'constellations,', len(pts), 'Milky Way points,', len(art), 'figure illustrations in art/')
